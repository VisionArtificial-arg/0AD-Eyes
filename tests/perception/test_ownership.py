...
# The historical default palette, spelled out as explicit literals.
...
MIN_FRACTION = 0.02  # historical assign_ownership default
...
def test_blue_rectangle_is_self() -> None:
    ...
    owner, frac = assign_ownership(
        _wrap(img), ScreenBBox(x=10, y=10, width=40, height=40), PALETTE, MIN_FRACTION
    )
...
def test_ownership_mask_covers_expected_region() -> None:
    img = _frame()
    cv2.rectangle(img, (10, 10), (50, 50), BLUE, -1)
    blue_color = next(c for c in PALETTE.colors if c.name == "blue")
